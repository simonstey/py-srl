...
class AggregationElement:
    ...
    """
    # TODO: Define structure once spec is complete
    pass
...
def validate_rule_well_formedness(rule: Rule) -> None:
    ...
    for i, element in enumerate(rule.body.elements):
        if isinstance(element, TriplePattern):
            ...
            for var in [element.subject, element.predicate, element.object]:
                if isinstance(var, Variable) and var in assignment_vars:
                    raise WellFormednessError(
                        f"Assignment variable {var} appears in triple pattern at position {i}"
                    )
            ...
            if element.variable in assignment_vars:
                raise WellFormednessError(
                    f"Assignment variable {element.variable} is assigned multiple times"
                )
            ...
            if undefined:
                raise WellFormednessError(
                    f"Variables {undefined} in assignment expression are not yet defined"
                )
    ...
    if undefined_head:
        raise WellFormednessError(
            f"Variables {undefined_head} in rule head are not defined in body"
        )
...
def _extract_variables_from_expression(expr: Expression) -> set[Variable]:
    ...
    if isinstance(expr, Variable):
        ...
        return (_extract_variables_from_expression(expr.left) |
                _extract_variables_from_expression(expr.right))
